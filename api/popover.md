...
URL: https://ionicframework.com/docs/api/popover

Version: v8
[shadow](/docs/reference/glossary#shadow)A Popover is a dialog that appears on top of the current page. It can be used for anything, but generally it is used for overflow actions that don't fit in the navigation bar.
...
| **Description** | Dismiss the popover overlay after it has been presented. This is a no-op if the overlay has not been presented yet. If you want to remove an overlay from the DOM that was never presented, use the [remove](https://developer.mozilla.org/en-US/docs/Web/API/Element/remove) method. 
...
| **Parameters** | **event** : The event to position the popover relative to. 
