...
URL: https://ionicframework.com/docs/api/title

Version: v8
[shadow](/docs/reference/glossary#shadow)Title is a text component that sets the title for a [toolbar](/docs/api/toolbar) . It can be used to describe the screen or section a user is currently on or the app being used.

## Basic

## Collapsible Large Titles

The large title will display when the [content](/docs/api/content) is scrolled to the start of the scroll container. When the title is scrolled behind the header, the condensed title will fade in.
...
The [buttons](/docs/api/buttons) component can be used with the [`collapse`](/docs/api/buttons#collapse) property to additionally display in the header as the toolbar is collapsed.
...
For example, if we wanted to make a Title behave like an `h1` element, we would set `role="heading"` and `aria-level="1"` on the Title. This is necessary when using the [Focus Manager](/docs/developing/managing-focus#assistive-technology-focus-management) .
