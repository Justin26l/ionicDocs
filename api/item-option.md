...
URL: https://ionicframework.com/docs/api/item-option

Version: v8
[shadow](/docs/reference/glossary#shadow)The item option component is an button for a sliding item. It must be placed inside of [item options](/docs/api/item-options) . The `ionSwipe` event and the `expandable` property can be combined to create a full swipe action for the item.
