...
URL: https://ionicframework.com/docs/api/toggle

Version: v8
[shadow](/docs/reference/glossary#shadow)Toggles are switches that change the state of a single option. They can be switched on or off by pressing or swiping them. Toggles can also be checked programmatically by setting the `checked` property.
...
Toggles can also be used in a list view by using the [Item](/docs/api/item) and [List](/docs/api/list) components.
